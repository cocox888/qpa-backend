...
const handleRef = async (req, res, next) => {
  try {
    ...
    if (!confirmToken) {
      reusedRef = jwt.verify(refresh_token, process.env.JWT_REF_TOKEN, async (error, decoded) => {
    ...
    verifyRef = jwt.verify(refresh_token, process.env.JWT_REF_TOKEN, async (error, decoded) => {
      //Find user role
      ...
      if (!role_id) return res.status(404).json({ message: 'No Record Found!' });
      const access_token = jwt.sign({
        id: confirmToken.id,
        email: confirmToken.email,
        role_id: role_id,
        role: role
      }, process.env.JWT_ACCESS_TOKEN, { expiresIn: '600s' });
      const refresh_token = jwt.sign({
        id: confirmToken.id,
        email: confirmToken.email,
        role_id: role_id,
        role: role
      }, process.env.JWT_REFRESH_TOKEN, { expiresIn: '48h' });
