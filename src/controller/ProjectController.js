...
class ProjectController {
  ...
  async getAllProjectByUserEmail(req, res) {
    try {
      const Project = await ProjectService.getAllProjectByUserEmail(
        req.body.email
      );
      res.status(200).json(Project);
    } catch (error) {
  ...
  async getAllFavouriteProjects(req, res) {
    try {
      ...
      if (
        ...
        Projects = await ProjectService.getAllFavouriteProjectForClient(
          userinfo.id
        );
      }
      res.status(200).json(Projects);
  ...
  async addFavouriteProject(req, res) {
    try {
      const userinfo = getAuthenticatedUser(req);
      const message = await ProjectService.addFavouriteProject(
        userinfo,
        req.body.projectId
      );
  ...
  async removeFavouriteProject(req, res) {
    try {
      const userinfo = getAuthenticatedUser(req);
      const message = await ProjectService.removeFavouriteProject(
        userinfo,
        req.body.projectId
      );
      res.status(200).json(message);
